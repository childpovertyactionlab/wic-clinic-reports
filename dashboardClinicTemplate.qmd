...
fail_count <- df.clinic_fails %>%
  ...
  mutate(name = paste0("Phase ", phase),
         color = palette_cpal_main[paste0("phase", phase)],
         message = glue_col("{n} items in <b style='color:{color}'>{name}</b>")) %>%
  pull(message)
...
CSAT.text.clinic <- paste0("In ", CSAT.clinic.recent$Date[1], ", the customer satisfaction score was ", ifelse(bettermonth, " better", " worse"), " than ", clinic_name, "'s average.", ifelse(bettermonth, " Good work!", ""))
...
CSAT.text.agency <- paste0("Agency-wide, ", clinic_name, " brought the average for Dallas County clinics", ifelse(CSAT.clinic.recent$CSAT[1] > CSAT.average.now, " up. Nice!", " down. Keep working!"))
...
staffchart <- plot_ly(count_df, 
  ...
  layout(plot_bgcolor = "rgba(0,0,0,0)",
         ...
         yaxis = list(title = "",
                      ...
                      tickprefix = "   "), # add 3 spaces before y-label
         xaxis = list(title = "",
                      gridwidth = 2,
                      dtick = 1), 
         barmode = 'stack') %>% 
...
<div class="d-flex align-items-center">
    <div style="display: inline-block; border: 1px solid #2f2f2f; border-radius: 5px; padding: 10px; ">
        ...
        <div style="font-size: 1em; font-style: italic; color: #2f2f2f; font-family: 'Poppins', sans-serif; padding-top: 10px;">
            Number of participants: <b>`r staff.clinics %>% filter(Location == clinic_name) %>% pull(var=4) %>% format(big.mark = ",")`</b>
        </div>
    </div>
</div>
```
