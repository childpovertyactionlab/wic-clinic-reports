...
CSAT.charting <- CSAT.charting %>% mutate(pred.fill = ifelse(pred, NA, "black"))
...
ggplot(CSAT.charting, aes(x = Date)) +
  geom_line(aes(y = LA_avg), linetype = "dotted", color = "black", linewidth = 1.25) +
  geom_segment(data = subset(CSAT.charting, pred == TRUE),
               aes(xend = lead(Date), y = LA_avg, yend = lead(LA_avg)), 
               linetype = "dotted", color = "black", linewidth = 1.25) +
  geom_line(aes(y = Average), color = "blue", linewidth = 1.25) +
  geom_segment(aes(xend = Date, 
                   y = Average + delta,
                   yend = Average,
